...
1. **Run REST API**
   ...
   Optional flags include `--host`, `--port`, `--name`, `--graphql`, `--env`, and `--create-tables`.
...
3. **Install MCP in Claude Desktop**
   ...
   ```bash
   mdmodels install claude-desktop --config config.toml --name mdmodels
   ```

   This writes an MCP server entry to Claude Desktop config and runs it via `uv run mdmodels mcp`.
...
If a model is omitted in `rest.endpoints`, default REST behavior applies for that model.

This makes it easy to begin with full API generation and then tighten exposure model-by-model as your API surface evolves.
...
- `--create-tables` is enabled by default for `mdmodels rest`, which is convenient for development and first-time setup.
- For stricter production workflows, run schema setup as a controlled deployment step and disable table creation at startup.
- Use explicit `--host` and `--port` values in infrastructure environments to avoid accidental defaults.
